...
with st.sidebar:
    ...
    <div style="padding:8px 0 20px;">
        ...
        <div style="font-size:18px;font-weight:700;color:#eceef5;line-height:1.25;letter-spacing:-0.01em;">
    ...
    uploaded = st.file_uploader(
        ...
        help="Upload your user feedback CSV"
    )
    st.caption("📁 Upload your feedback CSV above")

    st.markdown("---")
    st.markdown("<div style='font-size:10px;color:#4e5370;font-family:IBM Plex Mono,monospace;margin-bottom:10px;letter-spacing:0.08em;text-transform:uppercase;'>Filters</div>", unsafe_allow_html=True)

    f_sentiment = st.multiselect("Sentiment",   ["Positive","Negative","Neutral","Mixed"])
    f_persona   = st.multiselect("Persona",     ["Student","Working Professional","Homemaker","Job Seeker","Business","Other"])
    f_category  = st.multiselect("Category",    ["Content Quality","Technical Bug","Engagement/Retention","Subscription/Paywall"])
    f_dropoff   = st.multiselect("Drop-off",    ["No Habit loop","No Motivation","Streak Break","Pricing high","Content too hard","Technical Bug","Less Engaging","UI/UX issues"])

    st.markdown("---")
    max_rows = st.slider("Rows to analyse", 10, 100, 25, 5, help="More rows = richer insights but slower")
    st.markdown(f"<div style='font-size:11px;color:#4e5370;margin-top:4px;'>~{max_rows*2}s at {max_rows} rows</div>", unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("<div style='font-size:10px;color:#4e5370;'>Built by Krishnam Parasrampuria</div>", unsafe_allow_html=True)


# ─────────────────────────────────────────────────────────────────────────────
# LANDING PAGE (no file uploaded)
# ─────────────────────────────────────────────────────────────────────────────
# Case 1: No file has been uploaded anywhere yet
if uploaded is None and "feedback_df" not in st.session_state:
    # LANDING PAGE HERO
    st.markdown("""
    <div class="hero">
        <div class="hero-glow"></div>
        <div class="hero-eyebrow">// REVENUE & RETENTION RECOVERY</div>
        <div class="hero-title">Transform raw feedback into<br><em>engineering clear action</em>.</div>
        <div class="hero-sub">Drop your raw user feedback datasets below to get started.</div>
    </div>
    """, unsafe_allow_html=True)

    # Render file uploader directly in the main screen area
    col_l, col_m, col_r = st.columns([1, 2, 1])
    with col_m:
        main_uploaded = st.file_uploader(
            "Select or drop your Feedback CSV here", type=["csv"],
            key="main_uploader"
        )
        if main_uploaded is not None:
            uploaded = main_uploaded
            st.rerun()

        st.markdown("<p style='text-align: center; color: var(--muted); font-size: 13px; margin: 20px 0;'>— OR —</p>",
                    unsafe_allow_html=True)

        if st.button("Simulate Framework with Mock Dataset", use_container_width=True):
            mock_data = {
                "Raw Feedback": [
                    "App completely frozen on loading interface since the latest patch.",
                    "Love the concept! Very smooth design and useful daily notification patterns.",
                    "Subscription barrier is pricing out lower income student communities.",
                    "I preferred using Duolingo because of their quick streaks feature loop."
                ],
                "Sentiment": ["Negative", "Positive", "Negative", "Neutral"],
                "Persona Type": ["Student", "Working Professional", "Student", "Job Seeker"],
                "Feedback Category": ["Technical Bug", "Engagement/Retention", "Subscription/Paywall",
                                      "Engagement/Retention"]
            }
            st.session_state["feedback_df"] = pd.DataFrame(mock_data)
            st.rerun()
...
if f_sentiment and "Sentiment" in df.columns:
    df = df[df["Sentiment"].isin(f_sentiment)]
if f_persona and "Persona Type" in df.columns:
    df = df[df["Persona Type"].isin(f_persona)]
if f_category and "Feedback Category" in df.columns:
    df = df[df["Feedback Category"].isin(f_category)]
...
# PAGE HEADER
# ─────────────────────────────────────────────────────────────────────────────
fname = uploaded.name.replace(".csv","").replace("_"," ").title()
st.markdown(f"""
<div class="pg-header">
            ...
            <div class="wl-card">
              <div class="wl-top">
                <div>
                  ...
                  <div class="wl-meta">{ps} · {age} · {ls} lessons · streak {st}</div>
                </div>
            ...
            <div class="feat-card">
              <div class="feat-top">
                ...
                <div class="feat-badges">
                  ...
                  <span class="feat-badge" style="background:{pc}18;color:{pc};border:1px solid {pc}40;">{pri} Priority</span>
                  <span class="feat-badge" style="background:{ec}18;color:{ec};border:1px solid {ec}40;">{eff} Effort</span>
                </div>
              </div>
